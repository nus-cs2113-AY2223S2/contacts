...
public class Contacts0 {
     ...
     *
     */
    private static final int MAX_ENRIES = 100;
    /**
     *
     */
    ...
    public static void main(String[] args) {
        ...
        startScreen();
        String[][] list = new String[MAX_ENRIES][3];
        int count = 0;
        while (true) {
            ...
            switch (commandType) {
                case "add":
                    ...
                    if (splitArgs.length == 3 // 3 arguments
                        ...
                        final int indexOfEmailPrefix = commandArgs.indexOf("e/");// name is leading substring up to
                                                                                 // first data prefix symbol
                        ...
                        final int indexOfEmailPrefix1 = commandArgs.indexOf("e/");// phone is last arg, target is from
                                                                                  // prefix to end of string
                        if (indexOfPhonePrefix1 > indexOfEmailPrefix1) {
                            result = commandArgs.substring(indexOfPhonePrefix1, commandArgs.length()).trim()
                                    .replace("p/", "");
                        } else {
                            result = commandArgs.substring(indexOfPhonePrefix1, indexOfEmailPrefix1).trim()
                                    .replace("p/", "");
                        }
                        person1[1] = result;
                        String result1;
                        final int indexOfPhonePrefix2 = commandArgs.indexOf("p/");
                        final int indexOfEmailPrefix2 = commandArgs.indexOf("e/");// email is last arg, target is from
                                                                                  // prefix to end of string
                        if (indexOfEmailPrefix2 > indexOfPhonePrefix2) {
                            result1 = commandArgs.substring(indexOfEmailPrefix2, commandArgs.length()).trim()
                                    .replace("e/", "");
                        } else {
                            result1 = commandArgs.substring(indexOfEmailPrefix2, indexOfPhonePrefix2).trim()
                                    .replace("e/", "");
                        }
                        person1[2] = result1;
                    ...
                    if (decodeResult == null) {
                        feedback = String.format("Invalid command format: %1$s " + (System.lineSeparator() + "|| ")
                                + "%2$s", "add",
                                String.format("%1$s: %2$s", "add", "Adds a person to contacts.")
                                        + (System.lineSeparator() + "|| ")
                                        + String.format("\tParameters: %1$s", "NAME "
                                                + "p/" + "PHONE_NUMBER "
                                                + "e/" + "EMAIL")
                                        + (System.lineSeparator() + "|| ")
                                        + String.format("\tExample: %1$s",
                ...
                case "list":
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        messageAccumulator.append('\t').append(String.format("%1$d. ", displayIndex))
                                .append(String.format("%1$s  Phone Number: %2$s  Email: %3$s", person[0], person[1],
                                        person[2]))
                ...
                case "clear":
                    ...
                    break;
                case "help":
                    feedback = (String.format("%1$s: %2$s", "add", "Adds a person to contacts.")
                            + (System.lineSeparator() + "|| ")
                            + String.format("\tParameters: %1$s", "NAME "
                                    + "p/" + "PHONE_NUMBER "
                                    + "e/" + "EMAIL")
                            ...
                            + (System.lineSeparator() + "|| ")
                            + (String.format("%1$s: %2$s", "clear", "Clears all contacts.")
                                    + (System.lineSeparator() + "|| ")
